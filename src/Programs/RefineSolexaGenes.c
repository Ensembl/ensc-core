    ...
    for (j=0; j<Vector_getNumElement(modelsByScore); j++) {
      ...
      for ( my $i = 0 ; $i < scalar(@$model) ; $i++ ) {
        if ( $model->[$i]->isa("Bio::EnsEMBL::DnaDnaAlignFeature") ) {
          ...
          $new_exons[$k-1]->end( $intron->start );
          $new_exons[$k+1]->start( $intron->end );
