...
int main(int argc, char *argv[]) {
  ...
  failedTests += ok(1,Vector_getNumElement(v1) == 3);
  ...
  failedTests += ok(2,Vector_getNumElement(v2) == 4);

  Vector_append(v1,v2);

  failedTests += ok(3,Vector_getNumElement(v1) == 7);

  Vector_reverse(v1);

  str = Vector_getElementAt(v1,0);

  failedTests += ok(4, !strcmp(str,"g"));
